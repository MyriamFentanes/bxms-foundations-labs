...
. Navigate to the `[process_server_external_route]/services/rest/server/containers` route to confirm the existence of the `mortgages` container:
+
image::images/mortgages-container-execution-server.png[]
+
[NOTE]
====
The `/services/rest/server/containers` endpoint of Process Server is a _secured resource_. This means that you need to present credentials to access the resource. You use the same credentials (`adminUser` for the username and `test1234!` for the password) that you used for entering Decision Central from the _Environment Setup Lab_ .
====
